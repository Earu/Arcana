-- The Alchemy Table: refines crystal dust into elemental variants and buys
-- dust back for coins.
...
if SERVER then
	util.AddNetworkString("Arcana_Alchemy_Open")
	util.AddNetworkString("Arcana_Alchemy_Craft")
	util.AddNetworkString("Arcana_Alchemy_Sell")

	resource.AddFile("materials/entities/arcana_alchemy_table.png")
	...
	function ENT:SpawnFunction(ply, tr, classname)
		...
		local ent = ents.Create(classname or "arcana_alchemy_table")
	...
	hook.Add("PlayerDisconnected", "Arcana_Alchemy_RateClear", function(ply)
		lastAction[ply:SteamID64()] = nil
	end)

	local function readTable(ply)
		if not IsValid(ply) or not rateOk(ply) then return end

		local ent = net.ReadEntity()
		if not IsValid(ent) or ent:GetClass() ~= "arcana_alchemy_table" then return end
		if ply:GetPos():DistToSqr(ent:GetPos()) > Arcana.Gardening.USE_RANGE ^ 2 then return end

		return ent
	end

	net.Receive("Arcana_Alchemy_Craft", function(_, ply)
		local G = Arcana.Gardening
		local ent = readTable(ply)
		local id = net.ReadString()
		local count = math.Clamp(net.ReadUInt(8), 1, 10)
		if not ent then return end

		local recipe = G.RecipeById[id]
		if not recipe then return end

		-- Verify the full multiplied cost first: taking ingredient by
		-- ingredient could otherwise consume half a recipe and fail.
		for item, amount in pairs(recipe.ingredients) do
			if Arcana.GetItemCount(ply, item) < amount * count then
		...
		for item, amount in pairs(recipe.ingredients) do
			Arcana.TakeItem(ply, item, amount * count, "Alchemy")
		end

		for item, amount in pairs(recipe.output) do
			Arcana.GiveItem(ply, item, amount * count, "Alchemy")
		end

		ent:EmitSound("ambient/levels/labs/electric_explosion5.wav", 60, 140, 0.5)
	end)

	net.Receive("Arcana_Alchemy_Sell", function(_, ply)
		local G = Arcana.Gardening
		local ent = readTable(ply)
		...
		local amount = math.min(want, Arcana.GetItemCount(ply, item))

		if amount <= 0 then
			Arcana.SendErrorNotification(ply, "Nothing to exchange")

			return
		end

		if not Arcana.TakeItem(ply, item, amount, "Alchemy exchange") then return end

		Arcana.GiveCoins(ply, amount * rate, "Alchemy exchange")
...
hook.Add("Initialize", "arcana_alchemy_items", function()
	local variants = {
